...
*{{box-sizing:border-box}}body{{margin:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:radial-gradient(circle at 22% 10%,#39281c 0,#17130f 42%,#0f0d0b 100%);color:var(--text)}}main{{max-width:1180px;margin:0 auto;padding:42px 24px 80px}}h1{{font-size:42px;margin:0 0 8px}}p{{color:var(--muted)}}.pill{{display:inline-block;padding:6px 10px;border-radius:999px;background:#3a3027;color:var(--gold);font-weight:700}}.grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin:26px 0}}.card{{border:1px solid var(--line);border-radius:22px;background:rgba(37,32,24,.94);padding:20px;box-shadow:0 18px 50px rgba(0,0,0,.22)}}.metric{{font-size:36px;font-weight:800;color:var(--gold)}}section{{margin-top:22px}}table{{width:100%;border-collapse:collapse}}th,td{{text-align:left;padding:11px 12px;border-bottom:1px solid #3c3026;vertical-align:top}}th{{color:var(--gold);font-size:13px;text-transform:uppercase;letter-spacing:.04em}}button{{background:#4d3929;color:var(--text);border:1px solid var(--line);border-radius:10px;padding:7px 11px;font-weight:700;cursor:pointer}}button.danger{{color:var(--danger)}}.actions{{display:flex;gap:8px;flex-wrap:wrap}}.muted{{color:var(--muted)}}.notice{{margin-top:18px;padding:14px 16px;border:1px solid var(--line);border-radius:16px;background:#2f281f;color:var(--gold)}}code{{color:var(--gold)}}@media(max-width:860px){{.grid{{grid-template-columns:1fr 1fr}}}}@media(max-width:560px){{.grid{{grid-template-columns:1fr}}}}
</style>
</head>
<body>
<main>
...
<p>Local-only queue for held actions, pending memory candidates, and read-only page summary proposals.</p>
{notice}
<div class="grid">
